...
export function project(state, { frame = "frame:default", lens = "lens:neutral", cursor, projection } = {}) {
  const spans = state.observations.flatMap((observation) => (observation.fields ?? []).map((field) => {
    const eventIds = [observation.event_id].filter(Boolean);
    return { span_id: id("span", { source: observation.source_id, field: field.field_id }), source_id: observation.source_id, field_id: field.field_id, axes: field.axes ?? [], block_id: field.block_id, event_ids: eventIds };
  }));
  ...
  return {
    ...
    parameters: [...state.hypotheses.accepted, ...state.hypotheses.competing, ...state.hypotheses.held].map((h) => ({ parameter_id: h.hypothesis_id ?? h.event_id, status: h.status ?? "held", evidence: h.evidence ?? null, cube: h.cube ?? null })),
...
export function readingSnapshot(state, { frame = "frame:default", lens = "lens:neutral", source_id } = {}) {
  ...
  const snapshot = {
    ...
    source_id,
    semantic_head: state.semanticHead,
    units: bundle.spans.map((span) => ({ unit_id: span.span_id, operator_events: span.event_ids, provenance: { source_id: span.source_id, field_id: span.field_id, block_id: span.block_id }, held: state.hypotheses.held.length > 0, alternatives: state.hypotheses.competing })),
  };
