...
export function validateSemanticEvent(value, name = "SemanticEvent") {
  ...
  if (!v.event_id.startsWith("event:sha256:")) fail(name, "event_id must be event:sha256:<64 hex>");
...
export function validateCommand(value, name = "Command") {
  ...
  const allowed = new Set(["observation.admit", "effect.result.admit", "hypothesis.hold", "hypothesis.supersede", "discovery.advance", "discovery.resume"]);
