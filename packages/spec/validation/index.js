...
import { isCurrentOperator } from "../operators/epoch.js";

const HASH_RE = /^sha256:[0-9a-f]{64}$/;
const EVENT_ID_RE = /^event:sha256:[0-9a-f]{64}$/;
const BLOCK_ID_RE = /^block:sha256:[0-9a-f]{64}$/;
...
export function validateObservationBlock(value, name = "ObservationBlock") {
  const v = object(value, name);
  if (v.schema !== "ObservationBlock@1") fail(name, "schema must be ObservationBlock@1");
  if (typeof v.block_id !== "string" || !BLOCK_ID_RE.test(v.block_id)) fail(name, "expected block:sha256 block_id");
  string(v.value_type, name, "value_type"); array(v.shape, name, "shape"); array(v.values, name, "values"); hash(v.content_hash, name, "content_hash");
  const expected = canonicalHashSync({ schema: v.schema, value_type: v.value_type, shape: v.shape, axis_order: v.axis_order, values: v.values, missing: v.missing, loss: v.loss });
  if (v.content_hash !== expected) fail(name, `content_hash mismatch: expected ${expected}`);
  if (v.block_id !== `block:${expected}`) fail(name, `block_id mismatch: expected block:${expected}`);
  return v;
}

export function validateObservationEnvelope(value, name = "ObservationEnvelope", { blocks } = {}) {
  const v = object(value, name);
  ...
  if (blocks !== undefined) {
    array(blocks, name, "blocks");
    const byId = new Map(blocks.map((block) => [validateObservationBlock(block).block_id, block]));
    for (const field of v.fields) if (!byId.has(field.block_id)) fail(name, `field ${field.field_id} references missing block ${field.block_id}`);
    const expected = canonicalHashSync(blocks.map((block) => block.content_hash).sort());
    if (v.blocks_hash !== expected) fail(name, `blocks_hash mismatch: expected ${expected}`);
  }
  return v;
}

export function validatePriorSnapshot(value, name = "PriorSnapshot", { content } = {}) {
  const v = object(value, name);
  ...
  if (content !== undefined) {
    const expected = canonicalHashSync(content);
    if (v.content_hash !== expected) fail(name, `content_hash mismatch: expected ${expected}`);
  }
  return v;
}
...
export function validateCommand(value, name = "Command") {
  ...
  const allowed = new Set(["observation.admit", "effect.result.admit", "hypothesis.hold", "hypothesis.accept", "hypothesis.compete", "hypothesis.supersede", "hypothesis.retract", "discovery.advance", "discovery.resume"]);
  if (!allowed.has(v.type)) fail(name, `unknown command type ${v.type}`);
  if (v.inputs !== undefined) array(v.inputs, name, "inputs");
  if (v.type === "observation.admit") validateObservationEnvelope(v.payload, "ObservationEnvelope", { blocks: v.blocks });
...
export function semanticEventId(event) {
  const { event_id, ...body } = object(event, "SemanticEvent");
  return `event:${canonicalHashSync(body)}`;
}

export function validateSemanticEventId(event, name = "SemanticEvent") {
  const expected = semanticEventId(event);
  if (event.event_id !== expected) fail(name, `event_id mismatch: expected ${expected}`);
  return event;
}
