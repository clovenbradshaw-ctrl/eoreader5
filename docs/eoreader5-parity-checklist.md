...
- [ ] **`src/turn/meta-route.js` — Born-measured free-speech routing** — **Not yet started.** `eoreader4.2:src/turn/meta-route.js` measures free-text against exemplar bases via `bornSalience`, gates each direction with `deriveNull`, and settles with `relax` — a real physics router, not regex. `deriveNull` itself now exists on the new side (see the next item), but `bornSalience`, `relax`, and the router that composes them do not. No router (ported or ad hoc) exists in either new repo yet.

- [x] **`deriveNull` (Born-null threshold derivation)** — **Rebuilt, generalized, and in active use.** `eoreader5:packages/engine/emergence/nulls/index.js` implements `deriveNull({nullSamples, observedStatistic, tailDirection, quantile, protocol})` — a pure, deterministic (no `Math.random`/`Date.now`) generic Born-null gate: given a null distribution built by an explicit perturbation of the actual data, it derives a threshold and reports pass/fail plus a p-value, retaining the null samples and the caller-supplied protocol description for full replay. `createSeededRng`/`seededShuffle` are provided alongside it for callers that need to generate perturbations deterministically. It is not a byte-for-byte port of `eoreader4.2:src/core/voidnull.js` (that file was never read against this change), but it implements the same role described in this checklist and in spec section 12.8 ("scoring rules measure, deriveNull decides"), and is now the load-bearing threshold primitive for the individuation gate (next item). `bornSalience` (`src/weave/chorus/born.js`) and `relax` (`src/weave/longgen/relax.js`) remain **not yet started** — `deriveNull` alone does not reconstruct `meta-route.js`.
...
- [x] **Individuation gate (mass × coupling + named/INS bit; field/emanon/protogon/holon typing; name-bind promotion)** — **Rebuilt against the newer v0.2 terrain-promotion spec, not a port of 4.2's five-way typing.** `eoreader5:packages/engine/referents/individuation.js` (`classifyIndividuationType`, `individuateReferent`, `applyNameBind`) types a referent from mass × coupling using Born-null thresholds (`packages/engine/emergence/nulls/index.js`, previous item) rather than hand-set constants, and additionally gates admission on boundary stability under re-segmentation (`packages/engine/emergence/boundaries/index.js`, spec section 13.3) — a referent that fails boundary stability remains `field`, not admitted, even if mass/coupling typed it otherwise. This is a four-way typing (`field`/`emanon`/`protogon`/`holon`) per the v0.2 spec, not 4.2's five-way `typeReferents`/`classifyReferent`; `subjShare` is carried through as `agency_signal` for record-keeping but does not itself gate. Schema: `packages/spec/schemas/individuation-result.schema.json` (`IndividuationResult@1`) plus `null-protocol.schema.json`, validated by `validateIndividuationResult`/`validateNullProtocol` in `@eoreader/spec`, cross-checked against real gate output in `packages/engine/referents/individuation.schema.test.js`. Not yet done: wiring this gate into `packages/engine/runner.js`'s event pipeline (it is a standalone, tested module today, the same status `emergence/evaluate` and `emergence/search` already had); computing mass/coupling/boundaries from raw observations (a row-specific extraction concern per spec 13.6, left to a future sense-organ/observation-normalization layer); `eoreaderapp` has no entity/cast panel yet. **Doc gap closed:** `docs/individuation-gate.md` now exists and is the canonical writeup (it was missing from both `eoreader4.2` and `eoreader5` before this change, despite being cited as a dependency).
